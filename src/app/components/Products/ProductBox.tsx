...
const ProductBox: React.FC<ProductType> = ({data}) => {
    ...
    return (
        <div className='mb-4'> 
            <div className='relative flex justify-center bg-white cursor-pointer transition-transform transform shadow-lg hover:scale-105' onMouseEnter={() => setDisplay("flex")} onMouseLeave={() => setDisplay("hidden")}>
                <Image width={320} height={320} className='h-80 pb-9 pt-4 px-4 object-contain' src={data.picture} loading='eager' alt="product1"/>
                <div className={`absolute ${display} gap-3 bottom-2 left-0 w-full justify-center`}>
